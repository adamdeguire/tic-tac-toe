...
Here, a user can alternate between the light and dark modes as seen in all application screenshots above. This is accomplished by setting CSS attributes of all relevant HTML elements via the **Query** ```.css()``` method. The CSS attribute ```transition``` assigned to all affected elements allows this change to happen smoothly over a defined period, rather than instantaneously.
<p>&nbsp;</p>
